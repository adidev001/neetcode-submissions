...
public:
    bool makesquare(vector<int>& matchsticks) {
        ...
        if (total % 4 != 0) return false;     // can't split evenly
        long long target = total / 4;

        sort(matchsticks.rbegin(), matchsticks.rend());  // big sticks first = earlier fail
    ...
    bool dfs(vector<int>& m, int i, vector<long long>& sides, long long target) {
        ...
        for (int s = 0; s < 4; s++) {
            ...
            if (s > 0 && sides[s] == sides[s-1]) continue;   // prune: duplicate bucket
            sides[s] += m[i];
